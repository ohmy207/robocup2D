...
bool Bhv_ZhBlock::execute(rcsc::PlayerAgent * agent) {
	...
	if (num < 7 && ball.x < -30.0 && ball.x > -40.0
			&& wm.countTeammatesIn(
					rcsc::Circle2D(rcsc::Vector2D(opp.x - 3.0, opp.y), 3.0), 3,
					false) > 2)
		return false;
...
bool Bhv_ZhBlock::doBlockMove2011(rcsc::PlayerAgent * agent) {
	...
	if (me.dist(blockPos) < 1.7 && targetLine.dist(me) < 1.0
		...
		return true;
			...
			&& ball.dist(opp) < 1.3 && me.dist(goal) < opp.dist(goal)
			&& targetLine.dist(me) < 1.0
			&& targetLine.dist(opp_static_pos) < 0.2 && timeAtBlockPoint > 1)
					{
		if (!rcsc::Body_GoToPoint(opp, 0.4, dashPower, 1, false, true).execute(
				agent))
				{
	...
	} else if (!rcsc::Body_GoToPoint(blockPos, 0.4, dashPower, 1, 100, true,
			30.0).execute(agent))
			{
		rcsc::AngleDeg bodyAngle = agent->world().ball().angleFromSelf();
