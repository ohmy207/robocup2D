...
Bhv_SetPlayFreeKick::doMove( PlayerAgent * agent )
{
    ...
    Vector2D posAgent = agent->world().self().pos();
    Vector2D posBall = agent->world().ball().pos();
    Vector2D posKicker = agent->world().teammatesFromBall().front()->pos();
    const PlayerPtrCont & tems = agent->world().teammatesFromSelf();
    const PlayerPtrCont::const_iterator t_end = tems.end();
    int DIST_SCAPE = 6.5;
    bool flag = true;
    ...
    Vector2D kickerPos = wm.teammatesFromBall().front()->pos();
          ...
          ++t )
    {
      ...
      if( tpos.dist(kickerPos) < 1.5 )
        continue;

      if( tpos.dist(kickerPos) < wm.self().pos().dist(kickerPos) )
      {
        amInearest = false;
        break;
      }
    ...
    if( amInearest )
    {
         target_point = posBall + Vector2D::polar2vector( DIST_SCAPE , ( target_point - posBall ).th() );
    }
