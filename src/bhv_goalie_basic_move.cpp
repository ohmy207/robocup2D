...
//        base_move_x = -48.5;
    ...
    double danger_move_x = -51.5;

//    double xTemp=wm.ball().pos().x;
//    if(xTemp>-12.0)
//    {
//    	base_move_x = -41.5;
//    	danger_move_x = -47.5;
...
//    }
    ...
    double _zgoalhw=ServerParam::i().goalHalfWidth();
    //double _zgTemp=0.0;
    //if(base_pos.absY()>15.0)
    //	_zgTemp=1.0;
    if ( base_pos.y > _zgoalhw + 3)  //zsq 原来是3
    {
    	Vector2D right_pole( - ServerParam::i().pitchHalfLength(),
    			_zgoalhw );
    	AngleDeg angle_to_pole = ( right_pole - base_pos ).th();

    	if ( -130.0 < angle_to_pole.degree()                 //zsq 原来是-140
    			&& angle_to_pole.degree() < -90.0 )
    	{
    		agent->debugClient().addMessage( "RPole" );
    		return Vector2D( danger_move_x, _zgoalhw);
    	}
    }
    else if ( base_pos.y < -_zgoalhw - 3 )
    {
    ...
    if ( base_pos.x < -ServerParam::i().pitchHalfLength() + 12.0   //zsq 原来是8
    		&& base_pos.absY() > ServerParam::i().goalHalfWidth()+2.0  )   //zsq 原来是2
    {
    	Vector2D target_point( base_move_x, ServerParam::i().goalHalfWidth()+2.0);
    	if ( base_pos.y < 0.0 )
    	{
    		target_point.y *= -1.0;
    	}
    ...
    //    if(base_pos.x<-44.0 && base_pos.absY()<9.0 && !wm.existKickableTeammate())
    //    {
    //    	return base_pos;
    //    }
    //zsq 还可以加一条沿球运动方向的直线
    //    if(wm.ball().pos().x<-42.0 && wm.ball().pos().absY()<12.0)
    //    {
    //    	Vector2D _zvTemp(wm.ball().pos().x+52.5,wm.ball().pos().y);
    //    	const Line2D ball_line( wm.ball().pos(), wm.ball().vel().th() );  //zsq 新加的
    //    	const Line2D player_line(wm.self().pos(),wm.ball().vel().th()+90.0);   //垂直于进攻方向的直线
    //    	if(std::fabs(wm.ball().vel().th().degree()-_zvTemp.th().degree())<30.0)
    //    	{
    //    		if(!wm.existKickableTeammate())
    //    		{
    //    			return wm.ball().pos()+wm.ball().vel();
    //    		}
    //    		else if ( wm.existKickableOpponent() )
    //    		{
    //    			return ball_line.intersection(player_line);
    //
    //    		}
    //    	}
    //
...
//---------------------------------------------------------//
    {
        const double x_back = 3.0; // tune this!!
...
//        if(base_pos.x<-40.0 && base_pos.absY()<10.0)
//        {
//        	AngleDeg _zdeTemp=wm.ball().vel().th();
//        	if((_zdeTemp.degree()>105.0 && _zdeTemp.degree()<180.0) ||
//        			(_zdeTemp.degree()>-180.0 && _zdeTemp.degree()<-105.0))
//        	{
//        		_zdeTemp+=90.0;
//        		Line2D ball_line1(ball_point,wm.ball().vel().th());
//        		Line2D defend_line(wm.self().pos(),_zdeTemp);
//        		Vector2D _zTemp(ball_line1.intersection(defend_line));
//        		if(_zTemp.x<-44.0 && _zTemp.y<7.0 &&_zTemp.y>-7.0)
//        		{
//        			return _zTemp;
//
//        		}
